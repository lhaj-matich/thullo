...
const InvitesListItem = ({type, author, users, boardName, boardCover, createdAt, id, onAccept, onCancel}: InviteItemProps) => {
        ...
        <HStack justifyContent="space-between" width="100%">
          <HStack>
            <AvatarGroup size="sm" max={4}>
              ...
              { type == "received" ? users?.map((user, index) => <Avatar key={index} name={user.fullname} src={createImageLink(user.profileImage)} />) : ""}
            </AvatarGroup>
            { type != "received" ? <Text fontWeight="600" color="#BDBDBD" fontSize="12px">{author?.fullname}</Text> : "" }
          </HStack>
          <HStack>
            { type == "received" ? <Button fontWeight="400" onClick={() => {if (onAccept) onAccept(id)}}>Accept</Button> : ""}
            <Button variant="private" onClick={() => onCancel(id)}>Cancel</Button>
          </HStack>
        </HStack>
      </VStack>
    </HStack>
